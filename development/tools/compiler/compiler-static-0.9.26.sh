...
# start module https://mdl.sh/development/module/compiler/module-compiler-0.9.26.sh
moduleCompiler() { (
set -eu
...
# start module https://mdl.sh/development/module/validate/module-validate-0.9.9.sh
moduleValidate() { (
set -eu
# check number of arguments
if [ "$#" -ne 2 ]; then
...
export DEBUG_NAMESPACE="MODULE_VALIDATE"

hashFunc="$(printf '%s' "$targetHash" | cut -d '-' -f1)"
srcHash="$(printf '%s' "$src" | moduleChecksum "$hashFunc")" || exit 65
if [ "$srcHash" != "$targetHash" ]; then
	debug "Hash mismatch: is '$srcHash', but should be '$targetHash'." 1
	exit 1
fi
) }
# end module https://mdl.sh/development/module/validate/module-validate-0.9.9.sh
...
# end module https://mdl.sh/development/module/compiler/module-compiler-0.9.26.sh
