...
runModuleTests() { (
	...
	target="65"
	result="0"
	"$modulesh" -s "https://mdl.sh/misc/hello-world/hello-world-1.0.1.sh" >/dev/null 2>&1 || result="$?" && true
	assertEqual "Invalid function name" "$result" "$target"

	# invalid checksum
	target="65"
	result="0"
	"$modulesh" "helloWorld" "https://mdl.sh/misc/hello-world/hello-world-1.0.1.sh" "invalid-checksum" >/dev/null 2>&1 || result="$?" && true
	...
	"$modulesh" "$funcName" "https://mdl.sh/misc/hello-world/hello-world-1.0.1.sh" "cksum-3769348439"
	...
	target="65"
	result="0"
	"$modulesh" "$funcName" "https://mdl.sh/misc/hello-world/hello-world-1.0.1.sh" "cksum-999" >/dev/null 2>&1 || result="$?" && true
	...
	funcName="varFunc"
	"$modulesh" "$funcName" "https://mdl.sh/development/module/core/spec/assets/variable-0.9.0.sh" "cksum-132771303"
