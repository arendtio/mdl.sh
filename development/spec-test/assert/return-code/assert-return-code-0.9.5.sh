...
# execute the command before the imports to avoid contermination through the imports
# result actually conterminates, but it should not be too bad
result="0"
output="$(eval "$3" 2>&1)" || result="$?" && true

# mandatory parameters (also below the command execution to avoid contermination)
name="$1"
target="$2"
cmd="$3"
