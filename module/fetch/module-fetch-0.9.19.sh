...
module "colonConfigEvaluated" "https://mdl.sh/colon-config/colon-config-evaluated-0.9.7.sh" "cksum-1377799856"
module "httpsGet" "https://mdl.sh/https-get/https-get-1.0.6.sh" "cksum-2736914142"
...
if [ "$executed" = "false" ] && ( command -v md5sum >/dev/null 2>&1 || command -v md5 >/dev/null 2>&1 ); then
	...
	if cachePointer="$(tmpDirByUserKeyword "module-cache" 2>/dev/null)"; then
		...
		# execute with cache
		src="$(cache -d "$cachePointer" -s "httpsGet '$url'")" || exit $?
		executed="true"
	else
