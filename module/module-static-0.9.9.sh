...
# start module https://mdl.sh/module/module-fetch-0.9.10.sh
moduleFetch() { (
set -eu
...
# dependencies
# start module https://mdl.sh/colon-config/colon-config-evaluated-0.9.5.sh
colonConfigEvaluated() { (
set -eu
# parameters are defined in colon-config

# dependencies
# start module https://mdl.sh/colon-config/colon-config-0.9.3.sh
colonConfig() { (
set -eu
if [ "$#" -lt 2 ]; then
...
# dependencies
# start module https://mdl.sh/colon-config/colon-value-0.9.1.sh
colonValue() { (
set -eu
# mandatory parameters
key=$1
...
# end module https://mdl.sh/colon-config/colon-value-0.9.1.sh
...
# end module https://mdl.sh/colon-config/colon-config-0.9.3.sh

value="$(colonConfig "$@")"
eval "printf '%s ' $value | sed 's/ \$//' " # to evaluate variables like: $XDG_DATA_HOME
printf '\n' # and terminate with a newline
) }
# end module https://mdl.sh/colon-config/colon-config-evaluated-0.9.5.sh
...
# end module https://mdl.sh/module/module-fetch-0.9.10.sh
