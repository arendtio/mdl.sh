...
# start module https://mdl.sh/module/module-fetch-0.9.11.sh
moduleFetch() { (
set -eu
...
# start module https://mdl.sh/https-get/https-get-1.0.2.sh
httpsGet() { (
set -eu
# mandatory:
url="$1"
...
if command -v curl >/dev/null 2>&1; then
	...
	host="$(echo "$url" | sed -e 's;^[^/]\{1,\}://;;' -e 's;[/:].*$;;')"
	path="$(echo "$url" | sed -e 's;^[^/]\{1,\}://[^/]\{1,\}/;/;')" # with leading /
	port="$(echo "$url" | sed -e 's;^[^/]\{1,\}://[^/]\{1,\}:\([0-9]\{1,\}\)/.*$;\1;')"
	if [ "$port" = "$url" ]; then
		port="443"
	fi
...
# end module https://mdl.sh/https-get/https-get-1.0.2.sh
# start module https://mdl.sh/error/error-1.0.1.sh
error() { (
set -eu
# mandatory:
...
# end module https://mdl.sh/module/module-fetch-0.9.11.sh
